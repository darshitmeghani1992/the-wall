...
begin
  insert into media_object_deletions(
    ...
    'ops:age-boundary','mark-media',
    'validated/dddddddd-dddd-4ddd-8ddd-dddddddddddd/eeeeeeee-eeee-4eee-8eee-eeeeeeeeeeee/full.m4a',
    'test',clock_timestamp()-interval '1 second',clock_timestamp()-interval '24 hours',clock_timestamp()
  ) returning id into d;
  if exists(select 1 from claim_media_object_deletions(5,'58000000-0000-4000-8000-000000000300') where deletion_id=d)
     or not exists(select 1 from media_object_deletions where id=d and state='failed' and attempt_count=0) then
    raise exception 'inclusive 24-hour terminal boundary failed';
  end if;
end $$;
