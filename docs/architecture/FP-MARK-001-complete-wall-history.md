...
**Status:** Revised proposal pending independent architecture rereview
...
1. Introduce `listWallMarks(wallId, cursor?)` with a 50-item visible page in `(pinned DESC, created_at DESC, id DESC)` order, active status, Wall ID filter, and existing RLS. Its cursor holds the last emitted row's `pinned`, `created_at`, and `id`. Validate the pinned boolean, semantic timestamp, and UUID before any raw PostgREST expression. Avoid a nested pinned-boundary predicate: query `pinned=true` first, with a 50+1 lookahead and the existing `(created_at,id)` predicate only if resuming within that phase. If the pinned phase cannot fill the page, query `pinned=false` for the remaining slots plus one lookahead, starting from its beginning. When resuming from a `pinned=false` cursor, query only that phase with the validated timestamp/ID predicate. Both queries use the same wall/status filters and deterministic two-column order. A pinned lookahead means the next cursor is pinned; otherwise, an unpinned lookahead means it is unpinned. If the last emitted row is pinned and there is no pinned lookahead but an unpinned lookahead, retain the pinned cursor so the next request crosses the boundary. If either read fails, fail the entire page and leave the previous list/cursor unchanged.
...
**Build readiness:** NOT READY until the independent architecture rereview accepts the updated boundary/target contract. A database index decision requires separate measurement and authorization; the proposed client-only slice does not include a migration.
